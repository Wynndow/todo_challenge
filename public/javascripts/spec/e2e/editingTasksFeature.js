describe('Editing a task', function() {
  ...
  it('allows a task to be edited via form when you click it', function() {
    element(by.className('completed-false')).click();
    element(by.model("task.text")).sendKeys(' And profit');
    element(by.buttonText('Save')).click();
    expect(element(by.className('completed-false')).getText()).toEqual('Make a to do list. And profit');
  });
  ...
  it('hides the task while being edited', function() {
    element(by.className('completed-false')).click();
    expect(element(by.className('completed-false')).isDisplayed()).toBe(false);
  });
